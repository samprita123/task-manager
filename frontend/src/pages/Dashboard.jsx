      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pb-12">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-surface/50 p-6 rounded-2xl border border-gray-800 hover:bg-surface transition group">
               <p className="text-xs text-gray-400 uppercase font-black group-hover:text-primary transition">Total Registry Members</p>
            ...
            <div className="bg-surface/50 p-6 rounded-2xl border border-gray-800 hover:bg-surface transition group">
               <p className="text-xs text-gray-400 uppercase font-black group-hover:text-green-500 transition">Assigned to Projects</p>
               <div className="flex items-center justify-between mt-2">
            ...
            <div className="bg-surface/50 p-6 rounded-2xl border border-gray-800 hover:bg-surface transition group">
               <p className="text-xs text-gray-400 uppercase font-black group-hover:text-yellow-500 transition">Awaiting Assignment</p>
               <div className="flex items-center justify-between mt-2">
               ...
               <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                     <BarChart data={barData} margin={{ left: -20, bottom: 20 }}>
                        ...
                        <XAxis dataKey="name" stroke="#D1D5DB" fontSize={10} axisLine={false} tickLine={false} angle={-45} textAnchor="end" />
            ...
            <div className="bg-surface rounded-3xl border border-gray-800 shadow-2xl overflow-x-auto">
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     <thead className="bg-gray-900/30 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        <tr>
                     ...
                     <tbody className="divide-y divide-gray-800">
                        {priorityMatrix.map(p => (
                           <tr key={p._id} onClick={() => handleProjectClick(p)} className="hover:bg-gray-800/10 transition group cursor-pointer">
                              ...
                              <td className="px-6 py-4 text-[10px] font-mono text-gray-300">{new Date(p.dueDate).toLocaleDateString()}</td>
                              <td className="px-6 py-4">
               ...
               <div className="space-y-8">
                  {dueProgress.map((p, i) => (
                     <div key={i} onClick={() => handleProjectClick(p)} className="space-y-2 cursor-pointer hover:bg-gray-800/30 p-4 -m-4 rounded-xl transition border border-transparent hover:border-gray-800">
                        ...
                        <p className="text-[10px] text-gray-400">Estimated due in {Math.ceil((new Date(p.dueDate) - new Date()) / (1000 * 60 * 60 * 24))} days</p>
                     </div>
                  ))}
